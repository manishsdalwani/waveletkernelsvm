            ...
            log2g = log2g_list(j);
                for k = 1:numLog2g2
                    ...
                    % setting up the MultiScale Mexican Hat Wavelet kernel for training data
                     rbfKernel = @(X,Y) (sigma2^2/(sigma2^2-sigma^2)).*exp(-sigma2.*pdist2(X,Y,'euclidean').^2)-(sigma^2/(sigma^2-sigma2^2)).*exp(-sigma.* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
...
rbfKernel2 = @(X,Y) (bestsigma2^2/(bestsigma2^2-bestsigma^2)).*exp(-bestsigma2.*pdist2(X,Y,'euclidean').^2)-(bestsigma^2/(bestsigma^2-bestsigma2^2)).*exp(-bestsigma .* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="Multiscale MexicanHat") during Risky Decision')
saveas(h,sprintf('ROC_striatum_MultiScaleMexicanHatSVM_Risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
end
...
filename = 'striatum_MultiScaleMexicanHat_SVM_boys_vs_girls_risky.xlsx';
...
saveas(h,sprintf('Striatum Confusion Matrix Striatum Multiscale Mexican Hat SVM risky Decision Boys vs Girls.png'));
