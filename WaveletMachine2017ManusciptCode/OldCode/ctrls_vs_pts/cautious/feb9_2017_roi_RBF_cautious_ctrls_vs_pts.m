...
saveas(h,sprintf('ROC_RBFSVM_cautious_controlspatients_%d.png',k)); % will create FIG1, FIG2,...
end
...
T = cell2table({'Accuracy', 'MeanAbsoluteError', 'Kappa', 'TP', 'FP', 'Precision', 'Recall', 'F1score', 'AUC', 'PredBoys', 'PredGirls'; mean_Accuracy, mean_absolute_error, kappa, TP_weighted, FP_weighted, Precision_weighted, Recall_weighted, Fscore_weighted, mean(auc_roc_final), round(Pred_controls), round(Pred_patients)});
...
filename = 'roi_RBF_SVM_controls_vs_patients.xlsx';
...
saveas(h,sprintf('Confusion Matrix RBF SVM Cautious Decision controls vs patients.png'));
