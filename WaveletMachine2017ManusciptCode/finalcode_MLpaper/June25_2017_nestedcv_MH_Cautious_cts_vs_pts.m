...
besthyperparameters=zeros(11,2);
elapsed_time =[];
...
for i = 1:numLog2c
        ...
        for j = 1:numLog2g
            log2g = log2g_list(j);
                    ...
                    MexHatKernel = @(X,Y) 2/sqrt(3)*pi^(1/4)*(1-sigma*pdist2(X,Y,'euclidean').^2).*exp(-0.5*sigma .* pdist2(X,Y,'euclidean').^2);
                    K =  [ (1:dataRowNumber)', MexHatKernel(SortedData(:,3:features(2)),SortedData(:,3:features(2)))];
                    cmd = ['-t 4 -q -v 2 -c ', num2str(2^log2c)]; % - v option is the nested CV loop
...
MexHatKernel2 = @(X,Y) 2/sqrt(3)*pi^(1/4)*(1-bestsigma*pdist2(X,Y,'euclidean').^2).*exp(-0.5*bestsigma .* pdist2(X,Y,'euclidean').^2);

% setting up the kernel for training data post optimal hyperparameter selection
K2 =  [ (1:numTrain)', MexHatKernel2(SortedData(trainRows ,3:features(2)),SortedData(trainRows ,3:features(2)))];
...
KK = [ (1:numTest)', MexHatKernel2(SortedData(testRows ,3:features(2)),SortedData(trainRows ,3:features(2)))];
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="MH") during Cautious Decision')
saveas(h,sprintf('ROI%s_ROC_MHSVM_Cautious_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
besthyperparameters(roi,:) = [bestc, bestsigma];
...
saveas(h,sprintf('ROI%sConfusion Matrix MH SVM Cautious Decision controls vs patients.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('MH_Cautious_controls_vs_patients.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_MH_Cautious_controls_vs_patients.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_MH_Cautious_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');