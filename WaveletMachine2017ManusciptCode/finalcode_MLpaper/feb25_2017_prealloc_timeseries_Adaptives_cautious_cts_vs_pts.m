...
saveas(h,sprintf('Wholebrain_ROC_AdaptiveSVM_Cautious_controlspatients_%d.png',k)); % will create FIG1, FIG2,...
end
...
datasave=[mean_Accuracy mean_absolute_error kappa TP_weighted FP_weighted Precision_weighted Recall_weighted Fscore_weighted mean(auc_roc_final) round(Pred_controls) round(Pred_patients)];
%fprintf(' Accuracy MeanAbsoluteError Kappa TP FP Precision Recall F1score AUC Predcontrols Predpatients\n')
%disp(datasave)

% Detailed measures
extradatasave=[100*bal_Accuracy precision_controls precision_patients Recall_controls Recall_patients  sensitivity_controls sensitivity_patients specificity_controls specificity_patients Fscore_controls Fscore_patients FP_rate_controls FP_rate_patients TP_rate_controls TP_rate_patients];
...
saveas(h,sprintf('WholeBrain Confusion Matrix Adaptive SVM Cautious Decision controls vs patients.png'));
...
filename = 'wholebrain_Adaptive_SVM_controls_vs_patients_Cautious.xlsx';
...
saveas(h,sprintf('wholebrain Confusion Matrix Adaptive SVM Cautious Decision controls vs patients.png'));
elapsed_time=toc
dlmwrite('timetaken_Adaptive_Cautious_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');