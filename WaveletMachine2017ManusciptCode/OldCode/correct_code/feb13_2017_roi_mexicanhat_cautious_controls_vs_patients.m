...
basedir = ('/Users/manishdalwani/Desktop/Wavelet_Machine/cts_decout_ar_riskyflip/correctrecentcode_Feb18/');
cd(basedir)
...
for i = 1:numLog2c
        ...
        for j = 1:numLog2g
            log2g = log2g_list(j);
                    ...
                    rbfKernel = @(X,Y) 2/sqrt(3)*pi^(1/4)*(1-sigma*pdist2(X,Y,'euclidean').^2).*exp(-0.5*sigma .* pdist2(X,Y,'euclidean').^2);
...
rbfKernel2 = @(X,Y) 2/sqrt(3)*pi^(1/4)*(1-bestsigma*pdist2(X,Y,'euclidean').^2).*exp(-0.5*bestsigma .* pdist2(X,Y,'euclidean').^2);
...
saveas(h,sprintf('ROI%s_ROC_MexicanHatSVM_cautious_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
filename = 'roi_MexicanHat_SVM_controls_vs_patients_cautious.xlsx';
...
saveas(h,sprintf('ROI%sConfusion Matrix MexicanHat SVM Cautious Decision controls vs patients.png',rois));
end
