        ...
        for j = 1:numLog2g
            log2g = log2g_list(j);
                for ctrfreq = 0.1:4.1
                    ...
                    K =  [ (1:dataRowNumber)', morletKernel(SortedData(:,3:features(2)),SortedData(:,3:features(2)))];
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="Morlet") during Cautious Decision')
saveas(h,sprintf('ROI%s_ROC_MorletSVM_Cautious_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
besthyperparameters(roi,:) = [bestc, bestsigma, bestctrfreq];
...
saveas(h,sprintf('ROI%sConfusion Matrix Morlet SVM Cautious Decision controls vs patients.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('Morlet_Cautious_controls_vs_patients.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_Morlet_Cautious_controls_vs_patients.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_Morlet_Cautious_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');