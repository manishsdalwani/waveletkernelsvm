...
% BOYS VS. GIRLS
% OLDER SCRIPT ORIGINAL STRIATUM WITHOUT COREGISTRATION
% NAN'S discounted and size drops to 1124
...
for s=1:numsubjects
   ...
   %part where we read the images and apply mask
    VI = spm_vol(['con_0002_' num2str(subject) '.img']);
    [fmri_vol, fmri_xyz] = spm_read_vols(VI);
    fmri_masked_vector = fmri_vol(mask_vol);
    VII = reshape(fmri_masked_vector, mask_hdr.dim(1),mask_hdr.dim(2),mask_hdr.dim(3));
    VO=VII
    VO.fname = ['constriatum_0002_' num2str(subject) '.img'];
    VO = spm_create_image(VO);
   for i=1:VII.dim(3),
    img = spm_slice_vol(VII, spm_matrix([0 0 i]), VII.dim(1:2),0);
    tmp = find(isnan(img));
    img(tmp) = 0;
    VO = spm_write_plane(VO,img,i);
   end;
    subjfmri = ['constriatum_0002_' num2str(subject) '.img'];
    
    
    fmri_hdr = spm_vol(subjfmri);  % load fMRI volume
    [fmri_vol, fmri_xyz] = spm_read_vols(fmri_hdr);
    CURRENTsub = CURRENTsub + 1;
    ALLsubs(CURRENTsub,:) = reshape(fmri_vol, 1, 2847);
end;
