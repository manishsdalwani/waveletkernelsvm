% Boys vs girls
% Cautious
% Multiscale Mexican Hat Kingsbury
...
for log2c = -3.1:3.1,
    for d = 1:1:4
        for log2g = -20.1:20.1,
            for log2g2 = -20.1:20.1
                sigma=2^log2g;
                sigma2=2^log2g2;
                g2n=(1/sqrt(sigma)).^d;
                g2d=(1/sqrt(sigma)).^d-(1/sqrt(sigma2)).^d
                g2=g2n/g2d;
                g1n=(1/sqrt(sigma2)).^d;
                g1=g1n/g2d;
                rbfKernel = @(X,Y) g2.*exp(-sigma2.*pdist2(X,Y,'euclidean').^2)-g1.*exp(-sigma.* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
                ...
                if (cv >= bestcv),
                bestcv = cv; bestc = 2^log2c; bestsigma = 2^log2g; bestsigma2 = sigma2; bestg2=g2; bestg1=g1;
                end
                fprintf('%g %g %g (best c=%g, sigma=%g, rate=%g, sigma2=%g, g2=%g, g1=%g)\n', log2c, log2g, cv, bestc, bestsigma, bestcv, bestsigma2, bestg2, bestg1); 
            end
        end
    end
end
rbfKernel2 = @(X,Y) bestg2.*exp(-bestsigma2.*pdist2(X,Y,'euclidean').^2)-bestg1.*exp(-bestsigma .* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
