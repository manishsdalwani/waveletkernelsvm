        ...
        for j = 1:numLog2g
            log2g = log2g_list(j);
                for ctrfreq = 0.1:5.1
                    ...
                    rbfKernel = @(X,Y) cos(ctrfreq*sqrt(sigma).* pdist2(X,Y,'euclidean')).*exp(-sigma .* pdist2(X,Y,'euclidean').^2)-exp(-0.5*ctrfreq*ctrfreq).*exp(-sigma .* pdist2(X,Y,'euclidean').^2);     
...
rbfKernel2 = @(X,Y) cos(bestctrfreq*sqrt(bestsigma).* pdist2(X,Y,'euclidean')).*exp(-bestsigma .* pdist2(X,Y,'euclidean').^2)-exp(-0.5*bestctrfreq*bestctrfreq).*exp(-bestsigma .* pdist2(X,Y,'euclidean').^2);  
...
saveas(h,sprintf('ROI10_ROC_MorletSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
end
...
T3 = cell2table({'BESTC', 'BESTsigma', 'BESTCtrFrq'; bestc, bestsigma, bestctrfreq});
      %writetable helps to save the file into an excel file
filename = 'roi_ROI10_MORLET_SVM_boys_vs_girls_risky.xlsx';
...
saveas(h,sprintf('ROI10 Confusion Matrix Morlet SVM Risky Decision Boys vs Girls.png'));
