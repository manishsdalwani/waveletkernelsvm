...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="Adaptive") during Risky Decision')
saveas(h,sprintf('ROI%s_ROC_AdaptiveSVM_Risky_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
saveas(h,sprintf('ROI%sConfusion Matrix Adaptive SVM Risky Decision controls vs patients.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('Adaptive_Risky_controls_vs_patients.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_Adaptive_Risky_controls_vs_patients.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_Adaptive_Risky_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');