...
mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_01.nii'); % load mask and convert to logical indexes w/ SPM tools
...
for i = 1:numLog2c
    log2c = log2c_list(i);
     ...
     K =  [ (1:numTrain)', (SortedData(trainRows, 3:features(2))*SortedData(trainRows, 3:features(2))')];
...
K2 =  [ (1:numTrain)', (SortedData(trainRows ,3:features(2))*SortedData(trainRows ,3:features(2))')];
%Regularization
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
saveas(h,sprintf('ROI1_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
end
...
T3 = cell2table({'BESTC'; bestc});
      %writetable helps to save the file into an excel file
      filename = 'roi_ROI1_LINEAR_SVM_boys_vs_girls_risky.xlsx';
...
saveas(h,sprintf('ROI1 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
...
numsubjects = length(subject_list);
% ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_02.nii'); % load mask and convert to logical indexes w/ SPM tools
% spm_read_vols can helps us read the image and store coordinate information
[mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
%logical function helps binarize the mask
mask_vol = logical(mask_vol);    % create binary mask of T/F values
...
crossValidationFolds = 9;
      ...
      K2 = [ (1:numTrain)', grammatrix2];
            ...
            ylabel('True positive rate')
            title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
            saveas(h,sprintf('ROI2_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
            end
            ...
            T3 = cell2table({'BESTC'; bestc});
            %writetable helps to save the file into an excel file
            filename = 'roi_ROI2_LINEAR_SVM_boys_vs_girls_risky.xlsx';
            writetable(T, filename,'sheet',1);
            writetable(T2, filename,'sheet',2);
            writetable(T3, filename,'sheet',3);
            ...
            SVMpred = reshape(SVMpred, dataRowNumber, 1);
            plotconfusion(groundtruth',SVMpred');
            saveas(h,sprintf('ROI2 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
            ...
            numsubjects = length(subject_list);
            % ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
            mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_03.nii'); % load mask and convert to logical indexes w/ SPM tools
            % spm_read_vols can helps us read the image and store coordinate information
            [mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
            %logical function helps binarize the mask
            mask_vol = logical(mask_vol);    % create binary mask of T/F values
            ...
            K2 = [ (1:numTrain)', grammatrix2];
                  ...
                  ylabel('True positive rate')
                  title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                  saveas(h,sprintf('ROI3_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                  end
                  ...
                  T3 = cell2table({'BESTC'; bestc});
                  %writetable helps to save the file into an excel file
                  filename = 'roi_ROI3_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                  writetable(T, filename,'sheet',1);
                  writetable(T2, filename,'sheet',2);
                  writetable(T3, filename,'sheet',3);
                  ...
                  SVMpred = reshape(SVMpred, dataRowNumber, 1);
                  plotconfusion(groundtruth',SVMpred');
                  saveas(h,sprintf('ROI3 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
                  ...
                  K2 = [ (1:numTrain)', grammatrix2];
                        ...
                        ylabel('True positive rate')
                        title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                        saveas(h,sprintf('ROI4_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                        end
                        ...
                        T3 = cell2table({'BESTC'; bestc});
                        %writetable helps to save the file into an excel file
                        filename = 'roi_ROI4_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                        writetable(T, filename,'sheet',1);
                        writetable(T2, filename,'sheet',2);
                        writetable(T3, filename,'sheet',3);
                        ...
                        SVMpred = reshape(SVMpred, dataRowNumber, 1);
                        plotconfusion(groundtruth',SVMpred');
                        saveas(h,sprintf('ROI4 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
                        ...
                        numsubjects = length(subject_list);
                        % ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
                        mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_05.nii'); % load mask and convert to logical indexes w/ SPM tools
                        % spm_read_vols can helps us read the image and store coordinate information
                        [mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
                        %logical function helps binarize the mask
                        mask_vol = logical(mask_vol);    % create binary mask of T/F values
                        ...
                        K2 = [ (1:numTrain)', grammatrix2];
                              ...
                              ylabel('True positive rate')
                              title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                              saveas(h,sprintf('ROI5_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                              end
                              ...
                              T3 = cell2table({'BESTC'; bestc});
                              %writetable helps to save the file into an excel file
                              filename = 'roi_ROI5_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                              writetable(T, filename,'sheet',1);
                              writetable(T2, filename,'sheet',2);
                              writetable(T3, filename,'sheet',3);
                              ...
                              SVMpred = reshape(SVMpred, dataRowNumber, 1);
                              plotconfusion(groundtruth',SVMpred');
                              saveas(h,sprintf('ROI5 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
                              ...
                              numsubjects = length(subject_list);
                              % ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
                              mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_06.nii'); % load mask and convert to logical indexes w/ SPM tools
                              % spm_read_vols can helps us read the image and store coordinate information
                              [mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
                              %logical function helps binarize the mask
                              mask_vol = logical(mask_vol);    % create binary mask of T/F values
                              ...
                              K2 = [ (1:numTrain)', grammatrix2];
                                    ...
                                    ylabel('True positive rate')
                                    title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                                    saveas(h,sprintf('ROI6_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                                    end
                                    ...
                                    T3 = cell2table({'BESTC'; bestc});
                                    %writetable helps to save the file into an excel file
                                    filename = 'roi_ROI6_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                                    writetable(T, filename,'sheet',1);
                                    writetable(T2, filename,'sheet',2);
                                    writetable(T3, filename,'sheet',3);
                                    ...
                                    SVMpred = reshape(SVMpred, dataRowNumber, 1);
                                    plotconfusion(groundtruth',SVMpred');
                                    saveas(h,sprintf('ROI6 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
                                    ...
                                    numsubjects = length(subject_list);
                                    % ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
                                    mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_07.nii'); % load mask and convert to logical indexes w/ SPM tools
                                    % spm_read_vols can helps us read the image and store coordinate information
                                    [mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
                                    %logical function helps binarize the mask
                                    mask_vol = logical(mask_vol);    % create binary mask of T/F values
                                    ...
                                    K2 = [ (1:numTrain)', grammatrix2];
                                          ...
                                          ylabel('True positive rate')
                                          title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                                          saveas(h,sprintf('ROI7_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                                          end
                                          ...
                                          T3 = cell2table({'BESTC'; bestc});
                                          %writetable helps to save the file into an excel file
                                          filename = 'roi_ROI7_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                                          writetable(T, filename,'sheet',1);
                                          writetable(T2, filename,'sheet',2);
                                          writetable(T3, filename,'sheet',3);
                                          ...
                                          SVMpred = reshape(SVMpred, dataRowNumber, 1);
                                          plotconfusion(groundtruth',SVMpred');
                                          saveas(h,sprintf('ROI7 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
                                          ...
                                          numsubjects = length(subject_list);
                                          % ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
                                          mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_08.nii'); % load mask and convert to logical indexes w/ SPM tools
                                          % spm_read_vols can helps us read the image and store coordinate information
                                          [mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
                                          %logical function helps binarize the mask
                                          mask_vol = logical(mask_vol);    % create binary mask of T/F values
                                    ...
                                    % Nans were dropped from the images. Although it could be replaced with an insigificant number in
                                          ...
                                          K2 = [ (1:numTrain)', grammatrix2];
                                                ...
                                                ylabel('True positive rate')
                                                title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                                                saveas(h,sprintf('ROI8_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                                                end
                                                ...
                                                T3 = cell2table({'BESTC'; bestc});
                                                %writetable helps to save the file into an excel file
                                                filename = 'roi_ROI8_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                                                writetable(T, filename,'sheet',1);
                                                writetable(T2, filename,'sheet',2);
                                                writetable(T3, filename,'sheet',3);
                                                ...
                                                SVMpred = reshape(SVMpred, dataRowNumber, 1);
                                                plotconfusion(groundtruth',SVMpred');
                                                saveas(h,sprintf('ROI8 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
                                                % Clear Workspace
                                                clear all;
                                                % Clear command Line
                                                clc;
                                                ...
                                                numsubjects = length(subject_list);
                                                % ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
                                                mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_09.nii'); % load mask and convert to logical indexes w/ SPM tools
                                                % spm_read_vols can helps us read the image and store coordinate information
                                                [mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
                                                %logical function helps binarize the mask
                                                mask_vol = logical(mask_vol);    % create binary mask of T/F values
                                                ...
                                                K2 = [ (1:numTrain)', grammatrix2];
                                                      ...
                                                      ylabel('True positive rate')
                                                      title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                                                      saveas(h,sprintf('ROI9_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                                                      end
                                                      ...
                                                      T3 = cell2table({'BESTC'; bestc});
                                                      %writetable helps to save the file into an excel file
                                                      filename = 'roi_ROI9_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                                                      writetable(T, filename,'sheet',1);
                                                      writetable(T2, filename,'sheet',2);
                                                      writetable(T3, filename,'sheet',3);
                                                      ...
                                                      SVMpred = reshape(SVMpred, dataRowNumber, 1);
                                                      plotconfusion(groundtruth',SVMpred');
                                                      saveas(h,sprintf('ROI9 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
                                                      ...
                                                      numsubjects = length(subject_list);
                                                      % ROI mask upload. You can pick your own ROI. Note this particular section has SPM dependency
                                                      mask_hdr = spm_vol('rfDorsalFrontal_thr25_2mm_10.nii'); % load mask and convert to logical indexes w/ SPM tools
                                                      % spm_read_vols can helps us read the image and store coordinate information
                                                      [mask_vol, mask_xyz] = spm_read_vols(mask_hdr);
                                                      %logical function helps binarize the mask
                                                      mask_vol = logical(mask_vol);    % create binary mask of T/F values
                                                      ...
                                                      K2 = [ (1:numTrain)', grammatrix2];
                                                            ...
                                                            ylabel('True positive rate')
                                                            title('ROC for SVM Classification (Kernel="Linear") during Risky Decision')
                                                            saveas(h,sprintf('ROI10_ROC_linearSVM_risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
                                                            end
                                                            ...
                                                            T3 = cell2table({'BESTC'; bestc});
                                                            %writetable helps to save the file into an excel file
                                                            filename = 'roi_ROI10_LINEAR_SVM_boys_vs_girls_risky.xlsx';
                                                            writetable(T, filename,'sheet',1);
                                                            writetable(T2, filename,'sheet',2);
                                                            writetable(T3, filename,'sheet',3);
                                                            ...
                                                            SVMpred = reshape(SVMpred, dataRowNumber, 1);
                                                            plotconfusion(groundtruth',SVMpred');
                                                            saveas(h,sprintf('ROI10 Confusion Matrix Linear SVM Risky Decision Boys vs Girls.png'));
