% Ctrls vs Pts
% risky
% new striatum updated code
% Multiscale Mexican Hat
...
numTrain = size(SortedData(trainRows ,3:2849) ,1);
numTest = size(SortedData(testRows ,3:2849) ,1);

 bestcv = 0;
for log2c = -3.1:3.1,
  for log2g = -20.1:20.1,
      sigma=2^log2g;
     rbfKernel = @(X,Y) (16/15).*exp(-4*sigma.*pdist2(X,Y,'euclidean').^2)-(1/15).*exp(-sigma .* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
     K =  [ (1:numTrain)', rbfKernel(SortedData(trainRows ,3:2849),SortedData(trainRows ,3:2849))];
...
rbfKernel2 = @(X,Y) (16/15).*exp(-4*bestsigma.*pdist2(X,Y,'euclidean').^2)-(1/15).*exp(-bestsigma .* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
K2 =  [ (1:numTrain)', rbfKernel2(SortedData(trainRows ,3:2849),SortedData(trainRows ,3:2849))];
KK = [ (1:numTest)', rbfKernel2(SortedData(testRows,3:2849),SortedData(trainRows ,3:2849))];
...
%K3 =  [ (1:numTrain)', rbfKernel2(SortedData(trainRows ,3:2849),SortedData(trainRows ,3:2849))];
%KK2 = [ (1:81)', rbfKernel3(SortedData(:,3:2849),SortedData(:,3:2849))];
