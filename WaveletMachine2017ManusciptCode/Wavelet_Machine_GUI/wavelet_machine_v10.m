function varargout = wavelet_machine_v10(varargin)
% WAVELET_MACHINE_V10 MATLAB code for wavelet_machine_v10.fig
%      WAVELET_MACHINE_V10, by itself, creates a new WAVELET_MACHINE_V10 or raises the existing
%      singleton*.
%
%      H = WAVELET_MACHINE_V10 returns the handle to a new WAVELET_MACHINE_V10 or the handle to
%      the existing singleton*.
%
%      WAVELET_MACHINE_V10('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WAVELET_MACHINE_V10.M with the given input arguments.
%
%      WAVELET_MACHINE_V10('Property','Value',...) creates a new WAVELET_MACHINE_V10 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wavelet_machine_v10_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wavelet_machine_v10_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wavelet_machine_v10

% Last Modified by GUIDE v2.5 18-May-2017 10:01:47

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @wavelet_machine_v10_OpeningFcn, ...
                   'gui_OutputFcn',  @wavelet_machine_v10_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before wavelet_machine_v10 is made visible.
function wavelet_machine_v10_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wavelet_machine_v10 (see VARARGIN)

% Choose default command line output for wavelet_machine_v10
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes wavelet_machine_v10 wait for user response (see UIRESUME)
...
function varargout = wavelet_machine_v10_OutputFcn(hObject, eventdata, handles) 
    ...
    case 6
      ...
      case 7
          ...
          elseif (optimparm==1 && num_hyperparm==0)
              ...
              for i = 1:numLog2c
                  log2c = gridc(i);
                  % setting up the kernel for training data
                  K =  [ (1:dataRowNumber)', (SortedData(:, 3:features(2))*SortedData(:, 3:features(2))')];
                  cmd = ['-t 4 -q -v ', num2str(cvloop_parmselect), ' -c ', num2str(2^log2c)]; % - v option is the nested CV loop
                  cv = svmtrain(SortedData(:,2),K,cmd); %model fit
                  cvMatrix = [cvMatrix, cv]; %store fit for comparison
                  if (cv >= bestcv)
                      bestcv = cv; bestc = 2^log2c; % selection of optimal hyperparameter
                  end
           ...
           elseif (optimparm==0 && num_hyperparm==1)
              ...
              %hyperparm1value = evalin('base','hyperparm1value');
              hyperparm1value = handles.hyperparm1value;
              guidata(hObject,handles);
              assignin('base','hyperparm1value_new',hyperparm1value);
              bestc = evalin('base', 'bestc');
              %hyperparm1value = str2double(hyperparm1value);
              assignin('base',hyperparm1name,hyperparm1value);
              %sig=[];
              sig = hyperparm1value;
              assignin('base','sig', sig);
              %sig =eval(str2double(evalin('base','hyperparm1value')));
           ...
           elseif (optimparm==1 && num_hyperparm==1)
              ...
              for i = 1:numLog2c
                  ...
                  for j=1:numLog2g
                      log2g = grid1(j);
                      sigma=2^log2g;
                      % setting up the RBF Wavelet kernel for training data
...
%               num_lines = 1;
%               defaultans = {'exp(-sigma .* pdist2(X,Y).^2)','1','sigma','0.001','sigma2','0.01'};
...
%               dlg_title = 'Input Custom Kernel';
%               num_lines = 1;
%               defaultans = {'exp(-sigma .* pdist2(X,Y).^2)','1','sigma','-20:1:20','sigma2','-20:1:20'};
%               answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
%               assignin('base', 'answer', answer);
%               assignin('base', 'Kernel', answer{1});
%               assignin('base', 'bestc', answer{2});
...
switch listbox_index
    ...
    case 7
        ...
        elseif (optimparm==0 && num_hyperparm==1)
            ...
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'exp(-sigma .* pdist2(X,Y).^2)','1','sigma','0.001'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'bestc', answer{2});
         ...
         elseif (optimparm==1 && num_hyperparm==1)
            ...
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'exp(-sigma .* pdist2(X,Y).^2)','-10.1:1:9.9','sigma','-20:1:20'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'gridc', answer{2});
