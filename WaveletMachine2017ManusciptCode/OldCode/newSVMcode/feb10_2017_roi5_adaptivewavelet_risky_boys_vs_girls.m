        ...
        for j = 1:numLog2g
            log2g = log2g_list(j);
                for ctrfreq = 0.1:5.1
                    for gamma = 0.1:5.1
                        sigma=2^log2g;
                        % setting up the AdaptiveWavelet kernel for training data
                            rbfKernel = @(X,Y) exp(-gamma*(2-2*cos(ctrfreq*sqrt(sigma).* pdist2(X,Y,'euclidean')).*exp(-sigma .* pdist2(X,Y,'euclidean').^2)));%adaptive wavelet
                              ...
                              cvMatrix = [cvMatrix, cv]; %store fit for comparison
                                    ...
                                    fprintf('%g %g %g (best c=%g, g=%g, rate=%g, ctrfreq=%g, gamma=%g)\n', log2c, log2g, cv, bestc, bestsigma,bestcv, bestctrfreq, bestgamma);                    end
                end
        end
end

% The modified AdaptiveWavelet Kernel with the tuned parameters
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="AdaptiveWavelet") during Risky Decision')
saveas(h,sprintf('ROI5_ROC_striatum_AdaptiveWaveletSVM_Risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
end
...
T3 = cell2table({'BESTC', 'BESTsigma', 'BestCtrFreq', 'BestGamma'; bestc, bestsigma,bestctrfreq, bestgamma });
%writetable helps to save the file into an excel file
filename = 'roi_ROI5_AdaptiveWavelet_SVM_boys_vs_girls_risky.xlsx';
...
saveas(h,sprintf('ROI5 Confusion Matrix Striatum AdaptiveWavelet SVM risky Decision Boys vs Girls.png'));
