function varargout = wavelet_machine_v12(varargin)
% WAVELET_MACHINE_V12 MATLAB code for wavelet_machine_v12.fig
%      WAVELET_MACHINE_V12, by itself, creates a new WAVELET_MACHINE_V12 or raises the existing
%      singleton*.
%
%      H = WAVELET_MACHINE_V12 returns the handle to a new WAVELET_MACHINE_V12 or the handle to
%      the existing singleton*.
%
%      WAVELET_MACHINE_V12('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WAVELET_MACHINE_V12.M with the given input arguments.
%
%      WAVELET_MACHINE_V12('Property','Value',...) creates a new WAVELET_MACHINE_V12 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wavelet_machine_v12_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wavelet_machine_v12_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wavelet_machine_v12

% Last Modified by GUIDE v2.5 25-May-2017 16:03:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @wavelet_machine_v12_OpeningFcn, ...
    'gui_OutputFcn',  @wavelet_machine_v12_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before wavelet_machine_v12 is made visible.
function wavelet_machine_v12_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wavelet_machine_v12 (see VARARGIN)

% Choose default command line output for wavelet_machine_v12
handles.output = hObject;
sig = 0;
handles.hyperparm1value = sig;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes wavelet_machine_v12 wait for user response (see UIRESUME)
...
function varargout = wavelet_machine_v12_OutputFcn(hObject, eventdata, handles)
...
wholebrain = 0; %wholebrain option turned off at default
assignin('base','wholebrain',wholebrain);
optimparm = 0; %optimparm turned off
assignin('base','optimparm',optimparm);
sig=[];
assignin('base','sig', sig);
num_hyperparm=0;
assignin('base','num_hyperparm',num_hyperparm);
...
num_hyperparm = evalin('base','num_hyperparm');
optimparm = evalin('base', 'optimparm');
...
switch KernelChoice
    case 1
        if (optimparm==1)
            ...
            prompt = 'Enter the value for C parameter? ';
            bestc = inputdlg(prompt);
            bestc = bestc{1};
            %bestc = 1;
            assignin('base', 'bestc', bestc);
        end
    ...
    case 2
        ...
        if (optimparm==1)
            ...
            prompt = {'Enter the value for C parameter?','Enter the value for sigma parameter?'} ;
            hyperparm = inputdlg(prompt);
            assignin('base','hyperparm', hyperparm);
            bestc = hyperparm{1};
            bestsigma = str2num(hyperparm{2});
            assignin('base', 'bestc', bestc);
            assignin('base','bestsigma',bestsigma);
        end
    ...
    case 3
        if (optimparm==1)
            ...
            prompt = {'Enter the value for C parameter?','Enter the value for sigma parameter?', 'Enter the value of Center Frequency'} ;
            hyperparm = inputdlg(prompt);
            assignin('base','hyperparm', hyperparm);
            bestc = hyperparm{1};
            bestsigma = str2num(hyperparm{2});
            bestctrfreq = str2num(hyperparm{3});
            assignin('base', 'bestc', bestc);
            assignin('base','bestsigma',bestsigma);
            assignin('base', 'bestctrfreq', bestctrfreq);
        end
    ...
    case 5
        if (optimparm==1)
            ...
            prompt = {'Enter the value for C parameter?','Enter the value for sigma parameter?', 'Enter the value for sigma2 parameter'} ;
            hyperparm = inputdlg(prompt);
            assignin('base','hyperparm', hyperparm);
            bestc = hyperparm{1};
            bestsigma = str2num(hyperparm{2});
            bestsigma2 = str2num(hyperparm{3});
            assignin('base', 'bestc', bestc);
            assignin('base','bestsigma',bestsigma);
            assignin('base','bestsigma2',bestsigma2);
        end
    ...
    case 6
        if (optimparm == 1)
            ...
            prompt = {'Enter the value for C parameter?','Enter the value for sigma parameter?', 'Enter the value of Center Frequency', 'Enter the value of Gamma parameter'} ;
            hyperparm = inputdlg(prompt);
            assignin('base','hyperparm', hyperparm);
            bestc = hyperparm{1};
            bestsigma = str2num(hyperparm{2});
            bestctrfreq = str2num(hyperparm{3});
            bestgamma = str2num(hyperparm{4});
            assignin('base', 'bestc', bestc);
            assignin('base','bestsigma',bestsigma);
            assignin('base', 'bestctrfreq', bestctrfreq);
            assignin('base', 'bestgamma', bestgamma);
    ...
    case 7
        ...
        num_hyperparm = evalin('base','num_hyperparm');
        %hypername1value = 0;
        if(optimparm==0 && num_hyperparm==0)
            Kernel2 = evalin('base', 'Kernel');
            bestc = evalin('base', 'bestc');
            assignin('base','Kernel2',Kernel2);
            Kernel2 = str2func(Kernel2);
        elseif (optimparm==1 && num_hyperparm==0)
            Kernel = evalin('base', 'Kernel');
            Kernel = str2func(Kernel)
            gridc = evalin('base', 'gridc');
            Range = str2num(gridc)
            numLog2c = length(gridc);
            %numLog2g = length(log2g_list);
            cvMatrix = []; %will save the nested crossvalidation performance
            
            assignin('base','dataRowNumber',dataRowNumber);
            assignin('base','X',SortedData(:,3:features(2)));
            assignin('base','cvloop_parmselect',cvloop_parmselect);
            assignin('base', 'Range', Range);
            assignin('base','labels',SortedData(:,2));
            %[bestc, bestcv] = LinearHyperPar(Range, dataRowNumber, SortedData(:,3:features(2)), SortedData(:,3:features(2)),cvloop_parmselect, SortedData(:,2)); %
            %assignin('base', 'bestc', bestc);
            %For loop for grid search note it also includes a CV loop matching the CV for model performance
            %Recommendation is that C specifically depends on sample size hence to keep it simple picket the
            %whole data set
            ...
            for i = 1:length(Range)
                log2c = Range(i)
                K =  [ (1:dataRowNumber)', Kernel(SortedData(:,3:features(2)),SortedData(:,3:features(2)))];
                cmd = ['-t 4 -q -v ', num2str(cvloop_parmselect), ' -c ', num2str(2^log2c)]; % - v option is the nested CV loop
                cv = svmtrain(SortedData(:,2),K,cmd);
                if (cv >= bestcv)
            ...
            Kernel2 = evalin('base', 'Kernel');
            Kernel2 = str2func(Kernel2);
        elseif (optimparm==0 && num_hyperparm==1)
            hyperparm1name = evalin('base','hyperparm1name');
            hp1val = evalin('base','hyperparm1value');
            bestc = evalin('base', 'bestc');
            % bestc = handles.bestc;
            % guidata(hObject, handles);
            % hyperparm1name = handles.hyperparm1name;
            % guidata(hObject,handles);
            % hp1val = handles.hyperparm1value;
            % guidata(hObject,handles);
            hp1val = str2double(hp1val);
            assignin('base','hp1val_check',hp1val);
            assignin('base', 'bestc_check', bestc);
            assignin('base',hyperparm1name,hp1val);
            %fprintf('The best hyperparameters selected: %g (best c=%g, best ', hyperparm1name', '=%g)\n', bestc, hp1val);
            Kernel2 = evalin('base', 'Kernel');
            Kernel2 = str2func(Kernel2);
            assignin('base','Kernel2',Kernel2);
        elseif (optimparm==1 && num_hyperparm==1)
            Kernel = evalin('base', 'Kernel');
            Kernel = str2func(Kernel);
            assignin('base', 'Kernel', Kernel);
            gridc = evalin('base', 'gridc');
            gridc = str2num(gridc)
            grid1 = evalin('base', 'grid1');
            grid1 = str2num(grid1)
            numLog2c = length(gridc);
            numLog2g = length(grid1);
            cvMatrix = []; %will save the nested crossvalidation performance
            ...
            [bestc, bestsigma, bestcv] = OneHyperPar(gridc,grid1,Kernel,dataRowNumber,SortedData(:,3:features(2)),SortedData(:,3:features(2)),cvloop_parmselect,SortedData(:,2));
            assignin('base', 'bestc', bestc);
            assignin('base','bestsigma',bestsigma);
            hp1val = bestsigma;
            assignin('base','hyperparm1value',hp1val);
            Kernel2 = Kernel;
            %Kernel2 = evalin('base', 'Kernel');
            %Kernel2 = str2func(Kernel2);
        elseif (optimparm==0 && num_hyperparm==2)
            hyperparm1name = evalin('base','hyperparm1name');
            hp1val = evalin('base','hyperparm1value');
            hp1val = str2double(hp1val);
            hyperparm2name = evalin('base','hyperparm2name');
            hp2val = evalin('base','hyperparm2value');
            hp2val = str2double(hp2val);
            bestc = evalin('base', 'bestc');
            assignin('base','hp1val_check',hp1val);
            assignin('base', 'bestc_check', bestc);
            assignin('base',hyperparm1name,hp1val);
            assignin('base','hp2val_check',hp2val);
            assignin('base',hyperparm2name,hp2val);
            %fprintf('The best hyperparameters selected: %g (best c=%g, best ', hyperparm1name', '=%g)\n', bestc, hp1val);
            Kernel2 = evalin('base', 'Kernel');
            Kernel2 = str2func(Kernel2);
            assignin('base','Kernel2',Kernel2);
        elseif (optimparm==1 && num_hyperparm==2)
            Kernel = evalin('base', 'Kernel');
            Kernel = str2func(Kernel);
            assignin('base', 'Kernel', Kernel);
            gridc = evalin('base', 'gridc');
            ...
            grid1 = evalin('base', 'grid1');
            grid1 = str2num(grid1)
            grid2 = evalin('base', 'grid2');
            grid2 = str2num(grid2)
            numLog2c = length(gridc);
            numLog2g = length(grid1);
            numLog2g2 = length(grid2);
            cvMatrix = []; %will save the nested crossvalidation performance
            ...
            [bestc, bestsigma, bestsigma2, bestcv] = TwoHyperPar(gridc,grid1,grid2,Kernel,dataRowNumber,SortedData(:,3:features(2)),SortedData(:,3:features(2)),cvloop_parmselect,SortedData(:,2));
            assignin('base', 'bestc', bestc);
            assignin('base','bestsigma',bestsigma);
            assignin('base','bestsigma2',bestsigma2);
            hp1val = bestsigma;
            hp2val = bestsigma2;
            assignin('base','hyperparm1value',hp1val);
            assignin('base','hyperparm2value',hp2val);
            Kernel2 = Kernel;
            %Kernel2 = evalin('base', 'Kernel');
            %Kernel2 = str2func(Kernel2);
        end
...
for startOfRow = 1:numberOfRowsPerFold:dataRowNumber % middle element represents steps
    ...
    elseif (num_hyperparm==1)
        K2 =  [ (1:numTrain)', Kernel2(SortedData(trainRows ,3:features(2)),SortedData(trainRows ,3:features(2)), hp1val)];
    elseif (num_hyperparm==2)
        K2 =  [ (1:numTrain)', Kernel2(SortedData(trainRows ,3:features(2)),SortedData(trainRows ,3:features(2)), hp1val, hp2val)];
    else
        K2 =  [ (1:numTrain)', Kernel2(SortedData(trainRows ,3:features(2)),SortedData(trainRows ,3:features(2)), hp1val, hp2val, hp3val)];
    end
    assignin('base','K2',K2);
    %if (regyes == 1)
        %Regularization
        ...
        if (regyes ==1 && min(eig(grammatrix))<0)
        ...
        [~,p]=chol(grammatrix2);
        posdef(:,cvloop) = p;
        assignin('base','posdef',posdef);
        if (sum(posdef)==0)
            K2 = [ (1:numTrain)', grammatrix2];
            assignin('base','K2_new',K2);
        else
         msgbox(sprintf('Not RKHS and not positive definite, pick regularization'),'Error','Error');
         return
        end
    ...
    elseif (num_hyperparm==1)
        KK = [ (1:numTest)', Kernel2(SortedData(testRows,3:features(2)),SortedData(trainRows,3:features(2)), hp1val)];
    elseif (num_hyperparm==2)
        KK = [ (1:numTest)', Kernel2(SortedData(testRows,3:features(2)),SortedData(trainRows,3:features(2)), hp1val, hp2val)];
    elseif (num_hyperparm==3)
        KK = [ (1:numTest)', Kernel2(SortedData(testRows,3:features(2)),SortedData(trainRows,3:features(2)), hp1val, hp2val, hp3val)];
    end;
    assignin('base','KK',KK);
...
switch listbox_index
    ...
    case 7
        ...
        KernelChoice = 7;
        assignin('base', 'KernelChoice', KernelChoice);
        prompt = '# of Hyperparameters excluding C parameter';
        dlg_title = '# of Hyperparameters';
        num_lines = 1;
        num_hyperparm = inputdlg(prompt, dlg_title, num_lines);
        assignin('base','num_hyperparm', str2double(num_hyperparm{1}));
        num_hyperparm = str2double(num_hyperparm{1});
        if(optimparm==0 && num_hyperparm==0)
            prompt = {'Customized Kernel', 'Default C'};
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'exp(-sigma .* pdist2(X,Y).^2)','1'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'bestc', answer{2});
        elseif (optimparm==1 && num_hyperparm==0)
            prompt = {'Customized Kernel', 'Grid range for C in logbase2'};
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'exp(-sigma .* pdist2(X,Y).^2)','-10.1:1:9.9'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'gridc', answer{2});
        elseif (optimparm==0 && num_hyperparm==1)
            prompt = {'Customized Kernel', 'Default C','Hyperparameter 1 Name','Default Hyperparameter 1'};
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'exp(-hp1val.* pdist2(X,Y).^2)','1','sigma','0.001'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'bestc', answer{2});
            assignin('base', 'hyperparm1name', answer{3});
            assignin('base', 'hyperparm1value', num2str(answer{4}));
            assignin('base', strcat('best',answer{3}), num2str(answer{4}));
            % Handles for hyperparameter
            handles.bestc  = answer{2};
            guidata(hObject, handles);
            
            handles.hyperparm1name = answer{3};
            guidata(hObject, handles);
            
            handles.hyperparm1value = str2double(answer{4});
            guidata(hObject, handles);
        elseif (optimparm==1 && num_hyperparm==1)
            prompt = {'Customized Kernel', 'Grid range for C in logbase2','Hyperparameter 1 Name','Grid range for Hyperparamter 1 in logbase2'};
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'exp(-hp1val.* pdist2(X,Y).^2)','-10.1:1:9.9','sigma','-20:1:20'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'gridc', answer{2});
            assignin('base', 'hyperparm1name', answer{3});
            %assignin('base', strcat('grid',answer{3}), num2str(answer{4}));
            assignin('base', 'grid1', num2str(answer{4}));
        elseif (optimparm==0 && num_hyperparm==2)
            prompt = {'Customized Kernel', 'Default C','Hyperparameter 1 Name','Default Hyperparameter 1','Hyperparameter 2 Name','Default Hyperparameter 2'};
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'(hp2val/(hp2val-hp1val)).*exp(-hp2val.*pdist2(X,Y).^2)-(hp1val/(hp1val-hp2val)).*exp(-hp1val.* pdist2(X,Y).^2)','1','sigma','0.001','sigma2','0.01'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'bestc', answer{2});
            ...
            assignin('base', 'hyperparm1value', num2str(answer{4}));
            assignin('base', strcat('best',answer{3}), num2str(answer{4}));
            assignin('base', 'hyperparm2name', answer{5});
            assignin('base', 'hyperparm2value', num2str(answer{6}));
            assignin('base', strcat('best',answer{5}), num2str(answer{6}));
        elseif (optimparm==1 && num_hyperparm==2)
            prompt = {'Customized Kernel', 'Default C','Hyperparameter 1 Name','Grid range for Hyperparamter 1 in logbase2','Hyperparameter 2 Name','Grid range for Hyperparamter 2 in logbase2'};
            dlg_title = 'Input Custom Kernel';
            num_lines = 1;
            defaultans = {'(hp2val/(hp2val-hp1val)).*exp(-hp2val.*pdist2(X,Y).^2)-(hp1val/(hp1val-hp2val)).*exp(-hp1val.* pdist2(X,Y).^2)','-10.1:1:9.9','sigma','-20:1:20','sigma2','-20:1:20'};
            answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
            assignin('base', 'answer', answer);
            assignin('base', 'Kernel', answer{1});
            assignin('base', 'gridc', answer{2});
            assignin('base', 'hyperparm1name', answer{3});
            assignin('base', 'grid1', answer{4});
            assignin('base', strcat('grid',answer{3}), num2str(answer{4}));
            assignin('base', 'hyperparm2name', answer{5});
            assignin('base', 'grid2', answer{6});
            assignin('base', strcat('grid',answer{5}), num2str(answer{6}));
        end
...
% --- Executes on button press in optimizeparm.
function optimizeparm_Callback(hObject, eventdata, handles)
% hObject    handle to optimizeparm (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of optimizeparm
if (get(hObject,'Value') == get(hObject,'Max'))
    optimparm = 1 %optimparm turned on
else
    optimparm = 0 %optimparm turned off
end
assignin('base','optimparm',optimparm);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%winopen('WaveletMachineGUIHelp.pdf')
web('WaveletMachineGUIHelp.pdf', '-browser')
