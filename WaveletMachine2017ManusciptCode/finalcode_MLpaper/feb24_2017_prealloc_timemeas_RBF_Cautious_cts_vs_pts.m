...
% The modified RBF Wavelet Kernel with the tuned parameters
rbfKernel2 = @(X,Y) exp(-bestsigma .* pdist2(X,Y,'euclidean').^2);
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="RBF") during Cautious Decision')
saveas(h,sprintf('ROI%s_ROC_RBFSVM_Cautious_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
saveas(h,sprintf('ROI%sConfusion Matrix RBF SVM Cautious Decision controls vs patients.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('RBF_Cautious_controls_vs_patients.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_RBF_Cautious_controls_vs_patients.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_RBF_Cautious_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');