...
for log2c = -3.1:3.1,
  for log2g = -20.1:20.1,
      sigma=2^log2g;
     rbfKernel = @(X,Y) 2*sinc((2*sigma*pdist2(X,Y,'euclidean'))-sinc(sigma*pdist2(X,Y,'euclidean')));
...
rbfKernel2 = @(X,Y) 2*sinc((2*bestsigma*pdist2(X,Y,'euclidean'))-sinc(bestsigma*pdist2(X,Y,'euclidean')));
