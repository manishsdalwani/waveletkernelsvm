...
roifmri = 'rfDorsalFrontal_thr25_2mm_02.nii'; %specific roi
...
for i = 1:numLog2c
        ...
        for j = 1:numLog2g
            ...
            sigma=2^log2g;
                    ...
                    rbfKernel = @(X,Y) exp(-2^log2g.* pdist2(X,Y,'euclidean').^2);     
...
%bestc = 955.425783333691;
%bestsigma=1.9073486328125e-06;
...
%cmd = ['-t 2 -c ', num2str(bestc), ' -g ', num2str(bestsigma)]; % picked the best possible trade-off C
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="RBF") during RiskyDecision')
saveas(h,sprintf('ROC_RBFSVM_Cautious_BoysGirls_%d.png',k)); % will create FIG1, FIG2,...
end
...
datasave(1,:)=[mean_Accuracy mean_absolute_error kappa TP_weighted FP_weighted Precision_weighted Recall_weighted Fscore_weighted mean(auc_roc_final) round(Pred_Boys) round(Pred_Girls)];
%fprintf(' Accuracy MeanAbsoluteError Kappa TP FP Precision Recall F1score AUC PredBoys PredGirls\n')
%disp(datasave)

% Detailed measures
extradatasave(1,:)=[100*bal_Accuracy precision_Boys precision_Girls Recall_Boys Recall_Girls  sensitivity_Boys sensitivity_Girls specificity_Boys specificity_Girls Fscore_Boys Fscore_Girls FP_rate_Boys FP_rate_Girls TP_rate_Boys TP_rate_Girls];
...
%Final Data measures
finaldatasave(1,:)=[100*bal_Accuracy kappa Precision_weighted Recall_weighted Fscore_weighted auc_roc_final(2)];

%Best Hyperparameters
besthyperparameters(1,:) = [bestc, bestsigma];
...
saveas(h,sprintf('Confusion Matrix RBF SVM Risky Decision Boys vs Girls.png'));
elapsed_time = [elapsed_time toc];
dlmwrite('RBF_Risky_Boys_vs_Girls.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_RBF_Risky_Boys_vs_Girls.csv', besthyperparameters, 'Delimiter',',');
dlmwrite('timetakenperROI_RBF_Risky_Boys_vs_Girls.csv', elapsed_time, 'Delimiter',',');