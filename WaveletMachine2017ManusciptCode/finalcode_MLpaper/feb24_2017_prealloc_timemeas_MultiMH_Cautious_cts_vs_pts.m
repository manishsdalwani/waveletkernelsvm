...
besthyperparameters=zeros(11,3);
elapsed_time =[];
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="MultiMH") during Cautious Decision')
saveas(h,sprintf('ROI%s_ROC_MultiMHSVM_Cautious_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
besthyperparameters(roi,:) = [bestc, bestsigma, bestsigma2];
...
saveas(h,sprintf('ROI%sConfusion Matrix MultiMH SVM Cautious Decision controls vs patients.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('MultiMH_Cautious_controls_vs_patients.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_MultiMH_Cautious_controls_vs_patients.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_MultiMH_Cautious_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');