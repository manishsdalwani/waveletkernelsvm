...
correct_Boys=zeros(1,9);
incorrect_Boys=zeros(1,9);
correct_Girls=zeros(1,9);
incorrect_Girls=zeros(1,9);
cvloop = 0;
...
saveas(h,sprintf('ROI%s_ROC_AdaptiveSVM_Cautious_BoysGirls_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
datasave(roi,:)=[mean_Accuracy mean_absolute_error kappa TP_weighted FP_weighted Precision_weighted Recall_weighted Fscore_weighted mean(auc_roc_final) round(Pred_Boys) round(Pred_Girls)];
%fprintf(' Accuracy MeanAbsoluteError Kappa TP FP Precision Recall F1score AUC PredBoys PredGirls\n')
%disp(datasave)

% Detailed measures
extradatasave(roi,:)=[100*bal_Accuracy precision_Boys precision_Girls Recall_Boys Recall_Girls  sensitivity_Boys sensitivity_Girls specificity_Boys specificity_Girls Fscore_Boys Fscore_Girls FP_rate_Boys FP_rate_Girls TP_rate_Boys TP_rate_Girls];
...
saveas(h,sprintf('ROI%sConfusion Matrix Adaptive SVM Cautious Decision Boys vs Girls.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('Adaptive_Cautious_Boys_vs_Girls.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_Adaptive_Cautious_Boys_vs_Girls.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_Adaptive_Cautious_Boys_vs_Girls.csv', elapsed_time, 'Delimiter',',');