function varargout = wavelet_machine_v6(varargin)
% WAVELET_MACHINE_V6 MATLAB code for wavelet_machine_v6.fig
%      WAVELET_MACHINE_V6, by itself, creates a new WAVELET_MACHINE_V6 or raises the existing
%      singleton*.
%
%      H = WAVELET_MACHINE_V6 returns the handle to a new WAVELET_MACHINE_V6 or the handle to
%      the existing singleton*.
%
%      WAVELET_MACHINE_V6('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WAVELET_MACHINE_V6.M with the given input arguments.
%
%      WAVELET_MACHINE_V6('Property','Value',...) creates a new WAVELET_MACHINE_V6 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wavelet_machine_v6_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wavelet_machine_v6_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wavelet_machine_v6

% Last Modified by GUIDE v2.5 09-May-2017 10:43:47

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @wavelet_machine_v6_OpeningFcn, ...
                   'gui_OutputFcn',  @wavelet_machine_v6_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before wavelet_machine_v6 is made visible.
function wavelet_machine_v6_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wavelet_machine_v6 (see VARARGIN)

% Choose default command line output for wavelet_machine_v6
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes wavelet_machine_v6 wait for user response (see UIRESUME)
...
function varargout = wavelet_machine_v6_OutputFcn(hObject, eventdata, handles) 
...
switch KernelChoice
  case 1
      ...
      for i = 1:numLog2c
          ...
          cmd = ['-t 4 -q -v ', num2str(cvloop_modelperf), ' -c ', num2str(2^log2c)]; % - v option is the nested CV loop
            ...
            for log2g = -20.1:20.1,
                for ctrfreq = 0.1:5.1
                    for gamma = 0.1:5.1
                        ...
                        K =  [ (1:dataRowNumber)', Kernel(SortedData(:,3:features(2)),SortedData(:,3:features(2)))];
                        cmd = ['-t 4 -v ', num2str(cvloop_modelperf), ' -c ', num2str(2^log2c)];
