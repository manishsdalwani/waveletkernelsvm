...
for log2c = -3.1:3.1,
  for log2g = -20.1:20.1,
      ...
      sigma=2^log2g;
      sigma2=2^log2g2;
     rbfKernel = @(X,Y) (16/15).*exp(-sigma2.*pdist2(X,Y,'euclidean').^2)-(1/15).*exp(-sigma.* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
    ...
    if (cv >= bestcv),
      bestcv = cv; bestc = 2^log2c; bestsigma = 2^log2g; bestsigma2 = sigma2;
    end
    fprintf('%g %g %g (best c=%g, sigma=%g, rate=%g, sigma2=%g)\n', log2c, log2g, cv, bestc, bestsigma, bestcv, bestsigma2);
  end
end
end
rbfKernel2 = @(X,Y) (16/15).*exp(-bestsigma2.*pdist2(X,Y,'euclidean').^2)-(1/15).*exp(-bestsigma .* pdist2(X,Y,'euclidean').^2); %Multiscale Mexican Hat
