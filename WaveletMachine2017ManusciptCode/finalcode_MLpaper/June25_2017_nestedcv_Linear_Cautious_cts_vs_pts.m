...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="Linear") during Cautious Decision')
saveas(h,sprintf('ROI%s_ROC_LinearSVM_Cautious_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
saveas(h,sprintf('ROI%sConfusion Matrix Linear SVM Cautious Decision controls vs patients.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('Linear_Cautious_controls_vs_patients.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_Linear_Cautious_controls_vs_patients.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_Linear_Cautious_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');