function varargout = wavelet_machine_v3(varargin)
% WAVELET_MACHINE_V3 MATLAB code for wavelet_machine_v3.fig
%      WAVELET_MACHINE_V3, by itself, creates a new WAVELET_MACHINE_V3 or raises the existing
%      singleton*.
%
%      H = WAVELET_MACHINE_V3 returns the handle to a new WAVELET_MACHINE_V3 or the handle to
%      the existing singleton*.
%
%      WAVELET_MACHINE_V3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WAVELET_MACHINE_V3.M with the given input arguments.
%
%      WAVELET_MACHINE_V3('Property','Value',...) creates a new WAVELET_MACHINE_V3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wavelet_machine_v3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wavelet_machine_v3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wavelet_machine_v3

% Last Modified by GUIDE v2.5 08-May-2017 03:49:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @wavelet_machine_v3_OpeningFcn, ...
                   'gui_OutputFcn',  @wavelet_machine_v3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before wavelet_machine_v3 is made visible.
function wavelet_machine_v3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wavelet_machine_v3 (see VARARGIN)

% Choose default command line output for wavelet_machine_v3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes wavelet_machine_v3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);
kernel = {'Linear', 'RBF', 'Morlet', 'Mexican Hat', 'Multiscale Mexican Hat', 'MorletRBF'};

% load the cell array into the listbox (assumed to be named listbox1
set(handles.kernel,'String',kernel);

% --- Outputs from this function are returned to the command line.
function varargout = wavelet_machine_v3_OutputFcn(hObject, eventdata, handles) 
...
regyes = 1 % regularization is turned on
...
for s=1:numsubjects %numsubject is the total number of subjects
    ...
    ALLsubs(CURRENTsub,:) = reshape(fmri_vol, 1, 510340);
end;
...
%Final Data measures
finaldatasave=[100*bal_Accuracy kappa Precision_weighted Recall_weighted Fscore_weighted mean(auc_roc_final)];
...
T3 = cell2table({'BalancedAccuracy', 'Kappa','Precision', 'Recall', 'F1score', 'AUC'; 100*bal_Accuracy,kappa, Precision_weighted, Recall_weighted, Fscore_weighted, mean(auc_roc_final)});
...
set(handles.auc, 'String', num2str(mean(auc_roc_final)));
axes(handles.axes1);
imshow('ROC_SVM_1.png');
axes(handles.axes2);
