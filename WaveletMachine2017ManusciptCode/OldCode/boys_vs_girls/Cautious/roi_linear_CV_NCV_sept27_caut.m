...
h = figure;
ground_truth = SortedData(testRows,2);
...
[fpr,tpr,t,auc,optrocpt] = perfcurve(SortedData(testRows,2), decVals(:,1)*model.Label(1),model.Label(1));
k=figure;
plot(fpr,tpr);
...
fpr_roc(1:length(fpr),cvloop) = fpr;
tpr_roc(1:length(tpr),cvloop) = tpr;
t_roc (1:length(t),cvloop) = t;
auc_roc = [auc_roc, auc];
optrocpt_roc=[optrocpt_roc, optrocpt];
...
fpr_roc_final(:,1) = mean(fpr_roc(:,1:cvloop),2);
tpr_roc_final(:,1) = mean(tpr_roc(:,1:cvloop),2);
t_roc_final(:,1) = mean(t_roc(:,1:cvloop),2);
roc_plot = plot(fpr_roc_final,tpr_roc_final);
set(roc_plot,'linewidth',3);
%set(roc_plot(2),'linewidth',10);
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for Classification by SVM')
       
% AUC estimation
auc_roc_final = mean(auc_roc);
% Optimal ROC point estimation
optrocpc_final = mean(optrocpt_roc);
...
Fscore_boys = 2*mean_correct_boys/(2*mean_correct_boys+mean_incorrect_boys+mean_incorrect_girls)
Fscore_girls = 2*mean_correct_girls/(2*mean_correct_girls+mean_incorrect_girls+mean_incorrect_boys)
       
% FPR = FP/(FP+TN)
FP_rate_boys = mean_incorrect_boys/(mean_incorrect_boys+mean_correct_girls)
FP_rate_girls = mean_incorrect_girls/(mean_incorrect_girls+mean_correct_boys)
...
random_Accuracy = ((mean_correct_girls+mean_incorrect_boys)*(mean_correct_girls+mean_incorrect_girls)+(mean_incorrect_girls+mean_correct_boys)*(mean_incorrect_boys+mean_correct_boys))/(mean_correct_boys+mean_correct_girls+mean_incorrect_boys+mean_incorrect_girls)^2
...
datasave=[datasave; mean_Accuracy mean_absolute_error kappa TP_weighted FP_weighted Precision_weighted Recall_weighted Fscore_weighted auc_roc_final round(Pred_boys) round(Pred_girls)];
