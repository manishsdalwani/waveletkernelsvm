...
bestcv = 0;
%Morlet Wavelet application
morletKernel = @(sigma,ctrfreq) cos(ctrfreq*sqrt(sigma).* pdist2(SortedData(trainRows ,3:features(2)),SortedData(trainRows ,3:features(2)),'euclidean')).*exp(-sigma .* pdist2(SortedData(trainRows ,3:features(2)),SortedData(trainRows ,3:features(2)),'euclidean').^2)-exp(-0.5*ctrfreq*ctrfreq).*exp(-sigma .* pdist2(SortedData(trainRows ,3:features(2)),SortedData(trainRows ,3:features(2)),'euclidean').^2);   
log2g = -20:20;
sigma = 2.^log2g;
ctrfreq = 0.1:4.1;
% Using bsxfun for vectorization
gram = bsxfun(morletKernel, sigma, ctrfreq);
K = [(1:numTrain)', gram];
...
saveas(h,sprintf('Striatum_ROC_morletSVM_Risky_boysgirls_%d.png',k)); % will create FIG1, FIG2,...
end
toc     
...
filename = 'striatum_morlet_SVM_boys_vs_girls_Risky.xlsx';
...
saveas(h,sprintf('Striatum Confusion Matrix morlet SVM Risky Decision boys vs girls.png'));
