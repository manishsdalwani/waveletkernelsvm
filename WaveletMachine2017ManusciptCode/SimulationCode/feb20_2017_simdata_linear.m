...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="Linear")');
saveas(h,sprintf('simdata_ROC_LinearSVM_%d.png',k)); % will create FIG1, FIG2,...
end
%        
...
datasave=[datasave; mean_Accuracy mean_absolute_error kappa TP_weighted FP_weighted Precision_weighted Recall_weighted Fscore_weighted round(Pred_controls) round(Pred_patients)];
fprintf(' Accuracy MeanAbsoluteError Kappa TP FP Precision Recall F1score Predcontrols Predpatients\n')
disp(datasave)
...
filename = 'simdata_Linear_SVM.xlsx';
...
saveas(h,sprintf('simdata Confusion Matrix Linear SVM.png'));
