...
% Moved the NCV outside the CV loop
tic
% Clear Workspace
clearvars;
% Clear command Line
...
% List of ROIS
roi_list = {'01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11'};
...
% These matrices will capture useful performance parameters
datasave=zeros(11); 
extradatasave=zeros(11,15);
finaldatasave=zeros(11,6);
besthyperparameters=zeros(11,4);
elapsed_time =[];
% Execute the SVM kernel on each ROI
for roi=1:numROIs
tic
...
ALLsubs=zeros(81, size(fMRI_masked,1));
...
X = [-(ones(40, 1)), ALLsubs(1:40,:)]; % Add a column of minus ones to Allsubs
...
AccuracyData=zeros(1,9);
MeanAbsErr=zeros(1,9);
correct_controls=zeros(1,9);
incorrect_controls=zeros(1,9);
correct_patients=zeros(1,9);
incorrect_patients=zeros(1,9);
cvloop = 0;
fpr_roc_g1=zeros(10,9);
tpr_roc_g1=zeros(10,9);
t_roc_g1=zeros(10,9);
auc_roc_g1=zeros(1,9);
%optrocpt_roc_g1=[];
fpr_roc_g2=zeros(10,9);
tpr_roc_g2=zeros(10,9);
t_roc_g2=zeros(10,9);
auc_roc_g2=zeros(1,9);
%optrocpt_roc_g2=[];
grammatrix=[];
determinant=zeros(1,9);
lambda = zeros(1,9);
grammatrix2=[];
determinant2=zeros(1,9);
posdef=zeros(1,9);
...
for log2c = -3.1:3.1,
  for log2g = -20.1:20.1,
      for ctrfreq = 0.1:5.1
          for gamma = 0.1:5.1
      sigma=2^log2g;
     rbfKernel = @(X,Y) exp(-gamma*(2-2*cos(ctrfreq*sqrt(sigma).* pdist2(X,Y,'euclidean')).*exp(-sigma .* pdist2(X,Y,'euclidean').^2)));%adaptive wavelet
     K =  [ (1:dataRowNumber)', rbfKernel(SortedData(:,3:features(2)),SortedData(:,3:features(2)))];
     cmd = ['-t 4 -v 9 -c ', num2str(2^log2c)];
     cv = svmtrain(SortedData(:,2),K,cmd);
    if (cv >= bestcv),
      bestcv = cv; bestc = 2^log2c; bestsigma = 2^log2g; bestctrfreq = ctrfreq; bestgamma=gamma;
    end
    fprintf('%g %g %g (best c=%g, g=%g, rate=%g, ctrfreq=%g, gamma=%g)\n', log2c, log2g, cv, bestc, bestsigma,bestcv, bestctrfreq, bestgamma);
          end
      end
   end
end
...
% The modified Adaptive Wavelet Kernel with the tuned parameters
rbfKernel2 = @(X,Y) exp(-bestgamma*(2-2*cos(bestctrfreq*sqrt(bestsigma).* pdist2(X,Y,'euclidean')).*exp(-bestsigma .* pdist2(X,Y,'euclidean').^2)));%adaptive wavelet
...
correct_controls(:,cvloop) = C(1,1);
incorrect_controls(:,cvloop) = C(1,2);
correct_patients(:,cvloop)= C(2,2);
incorrect_patients(:,cvloop) = C(2,1);
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for SVM Classification (Kernel="Adaptive") during Cautious Decision')
saveas(h,sprintf('ROI%s_ROC_AdaptiveSVM_Cautious_controlspatients_%d.png',rois,k)); % will create FIG1, FIG2,...
end
...
datasave(roi,:)=[mean_Accuracy mean_absolute_error kappa TP_weighted FP_weighted Precision_weighted Recall_weighted Fscore_weighted mean(auc_roc_final) round(Pred_controls) round(Pred_patients)];
%fprintf(' Accuracy MeanAbsoluteError Kappa TP FP Precision Recall F1score AUC Predcontrols Predpatients\n')
%disp(datasave)

% Detailed measures
extradatasave(roi,:)=[100*bal_Accuracy precision_controls precision_patients Recall_controls Recall_patients  sensitivity_controls sensitivity_patients specificity_controls specificity_patients Fscore_controls Fscore_patients FP_rate_controls FP_rate_patients TP_rate_controls TP_rate_patients];
...
%Final Data measures
finaldatasave(roi,:)=[100*bal_Accuracy kappa Precision_weighted Recall_weighted Fscore_weighted mean(auc_roc_final)];

%Best Hyperparameters
besthyperparameters(roi,:) = [bestc, bestsigma, bestctrfreq, bestgamma];
...
saveas(h,sprintf('ROI%sConfusion Matrix Adaptive SVM Cautious Decision controls vs patients.png',rois));
elapsed_time = [elapsed_time toc];
end
dlmwrite('Adaptive_Cautious_controls_vs_patients.csv', finaldatasave, 'Delimiter',',');      
dlmwrite('bestparams_Adaptive_Cautious_controls_vs_patients.csv', besthyperparameters, 'Delimiter',',');
totaltime = toc;
dlmwrite('timetakenperROI_Adaptive_Cautious_controls_vs_patients.csv', elapsed_time, 'Delimiter',',');